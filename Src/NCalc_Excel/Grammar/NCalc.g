...
ID  /* change: added ExcelSymbol*/	
	: 	(LETTER | '$')  (LETTER | DIGIT | EXCELSYMBOL+ (LETTER | DIGIT))*
	;
	
EXCELSYMBOL 
/* change: so we can parse excel addresses */	
	:	'_'
	|	':' 
	|	'!' 
	|	'.'
...
STRING /*added " as a string delimiter*/
	:  	( '"' ( EscapeSequence | (options {greedy=false;} : ~('\u0000'..'\u001f' | '\\' | '"'  ) ) )* '"'  |  '\'' ( EscapeSequence | (options {greedy=false;} : ~('\u0000'..'\u001f' | '\\' | '\'' ) ) )* '\'' )
    	;
...
fragment LETTER
	:	'a'..'z'
	|	'A'..'Z'
	; /* change: moved _ into  ExcelSymbol*/	
